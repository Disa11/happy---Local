...
cred = credentials.Certificate('happytails-4f2c8-firebase-adminsdk-19x4g-3071ff2488.json')

firebase_admin.initialize_app(cred, {
    'databaseURL': 'https://happytails-4f2c8-default-rtdb.firebaseio.com/'
})
...
] = "postgresql://happytails_hqzn_user:tpI8M5Gu5oWT2a23Z3GRZ592eiXmnoEp@dpg-ct4sq23v2p9s738sdmv0-a.oregon-postgres.render.com/happytails_hqzn"
...
def user(username):
    if request.method == "GET":
        ...
        form.species.choices = [
            (species.id, species.name) for species in Species.query.all()
        ]
        ...
        pets = [
            (pet.name, pet.age, pet.location_id, pet.breed_id)
            for pet in Pet.query.filter_by(user_id=session["user_id"]).all()
        ]
        ...
        return render_template(
            "user.html", form=form, user=user, links=links, pets=pets, location=location
        )
    ...
    if form.validate_on_submit():
        ...
        if form.profile_picture.data:
            ...
            form.profile_picture.data.filename = (
                "user" + f"{session['user_id']}" + f"{file_extension}"
            )
    ...
    while background_task_running:
        ...
        try:
            if len(data) == 0:
                ...
                data["time"] = tiempo_a_segundos(data["time"])
...
if __name__ == "__main__":
    ...
    # Inicia la tarea de planificación en un hilo separado
